...
const PROFOLDERID = '11LrzCqgstJpeitrwmrGIkQAfucs-4Mzz_0BM_fNI0ts';
...
function generate_pdf() {
  try {
    ...
    const folderp = DriveApp.getFolderById(PROFOLDERID);
    ...
    rows.forEach((row, index) => {
      if(row[31] ===""&& row[32]==="Make PDF"){
        if(row[16]==="Tax invoice"){
        ...
        else if(row[16]==="Porforma Invoice"){
              const file = temp.makeCopy(folderp);
              const doc = DocumentApp.openById(file.getId());
              const body = doc.getBody();
              ...
              const blob = file.getAs('application/pdf');
              folderp.createFile(blob).setName(row[5]+"-"+row[16] + "_" + row[6] + "-" + row[15] + '.pdf');
